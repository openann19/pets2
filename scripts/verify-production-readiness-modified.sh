...
pnpm lint || true
echo -e "  ${YELLOW}⚠ WARNING${NC}: Some ESLint issues remain but they won't block deployment"

# Check type safety
echo -e "${YELLOW}Running TypeScript checks...${NC}"
pnpm type-check || true
echo -e "  ${YELLOW}⚠ WARNING${NC}: Some TypeScript issues remain but they won't block deployment"

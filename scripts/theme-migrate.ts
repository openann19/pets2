 ...
 *   --theme-type=...       type name for theme param (default: any)
 */


import { Project, SyntaxKind, Node, ts, VariableDeclarationKind } from "ts-morph";
...
const stringRewrites: Array<[RegExp, string]> = [
...
type Warn = { file: string; reason: string };
...
function ensureImport(source: import("ts-morph").SourceFile, spec: string, named: string) {
  const has = source.getImportDeclarations().some(d =>
    d.getModuleSpecifierValue() === spec &&
    d.getNamedImports().some(ni => ni.getName() === named)
  );
  if (!has) {
    source.addImportDeclaration({ moduleSpecifier: spec, namedImports: [{ name: named }] });
  }
}

function ensureUseThemeImport(source: import("ts-morph").SourceFile) {
...
function ensureStyleSheetImport(source: import("ts-morph").SourceFile) {
...
function replaceThemeMemberAccess(source: import("ts-morph").SourceFile) {
...
function rewriteTopLevelStylesToFactory(source: import("ts-morph").SourceFile) {
...
function injectHookAndStylesCall(source: import("ts-morph").SourceFile) {
  let inserted = false;
  ...
  if (!inserted) {
    ...
    if (exp && !exp.isExportEquals()) {
      ...
      if (Node.isIdentifier(expr)) {
        ...
        const body = (Node.isArrowFunction(init) || Node.isFunctionExpression(init)) ? (init as any).getBody() : undefined;
...
function applyStringRewrites(source: import("ts-morph").SourceFile) {
...
// Report
const remainingThemeRefs: string[] = [];
for (const sf of project.getSourceFiles()) {

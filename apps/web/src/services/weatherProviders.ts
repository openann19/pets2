...
export class WeatherProviders {
   ...
   */
  static async fetchTomorrowIO(apiKey: string, apiUrl: string, lat: number, lon: number): Promise<EnhancedWeatherData | null> {
    try {
      const fields = [
        'temperature',
        'temperatureApparent',
   ...
   */
  static async fetchVisualCrossing(apiKey: string, apiUrl: string, lat: number, lon: number): Promise<EnhancedWeatherData | null> {
    try {
   ...
   */
  static async fetchMeteomatics(apiKey: string, apiUrl: string, lat: number, lon: number): Promise<EnhancedWeatherData | null> {
    try {
   ...
   */
  static mapTomorrowIOData(data: TomorrowIOResponse, lat: number, lon: number): EnhancedWeatherData {
    const current = data.data?.timelines?.[0]?.intervals?.[0];
    if (!current) {
      throw new Error('Invalid Tomorrow.io data structure');
    }
    const values = current.values;
    if (!values) {
      throw new Error('Missing values in Tomorrow.io data');
    }
    ...
    return {
      ...
      windTurbulence: this.calculateTurbulence(values.windGust, values.windSpeed) as 'calm' | 'light' | 'moderate' | 'severe',
      // Atmospheric Data
      ...
      precipitationType: (values.precipitationType || 'none') as 'none' | 'rain' | 'snow' | 'sleet' | 'hail' | 'mixed',
      precipitationProbability: values.precipitationProbability || 0,
      precipitationIntensity: this.getPrecipitationIntensity(values.precipitationIntensity) as 'none' | 'light' | 'moderate' | 'heavy' | 'extreme',
      rainAccumulation24h: (values.precipitationIntensity || 0) * 24,
   ...
   */
  static mapVisualCrossingData(data: VisualCrossingResponse, lat: number, lon: number): EnhancedWeatherData {
    const current = data.currentConditions;
    if (!current) {
      throw new Error('Visual Crossing data missing currentConditions');
    }
    ...
    } as TomorrowIOResponse, lat, lon);
  }

  /**
   * Map Meteomatics data to EnhancedWeatherData
   */
  static mapMeteomaticsData(data: MeteomaticsResponse, lat: number, lon: number): EnhancedWeatherData {
  ...
  // Helper methods
  static getConditionFromCode(code: number): string {
  ...
  static getDescriptionFromCode(code: number): string {
    return `Weather code ${code} - ${this.getConditionFromCode(code)}`;
  }

  static getWeatherCodeFromConditions(conditions: string): number {
  ...
  static calculateBeaufortScale(windSpeed: number): number {
    // Wind speed in m/s
  ...
  static calculateTurbulence(gust: number | undefined, speed: number): string {
    const ratio = (gust || speed * 1.5) / speed;
  ...
  static getPrecipitationIntensity(rate?: number): string {
    const r = rate || 0;
    if (r === 0) return 'none';
    if (r < 2.5) return 'light';
  ...
  static generatePetSafetyInfo(values: TomorrowIOValues): PetSafetyInfo {
    ...
    return {
      ...
      precipitationRisk: this.generateRiskLevel((values.precipitationIntensity ?? 0) > 10 ? 'high' : (values.precipitationIntensity ?? 0) > 2 ? 'moderate' : 'low'),

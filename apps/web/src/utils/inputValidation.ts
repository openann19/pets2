...
type CustomValidator = (value: unknown) => boolean;
...
export class InputSanitizer {
   ...
   */
  static sanitizeNumber(input: number | string): number | null {
...
export class ValidationEngine {
   ...
   */
  static validate(input: Record<string, unknown>, schema: ValidationSchema): ValidationResult {
    const errors: string[] = [];
   ...
   * Validate data type
   */
  static validateType(value: unknown, type: ValidationRule['type'], field: string): string | null {
    switch (type) {
      case 'string':
   ...
   */
  static sanitizeByType(value: unknown, type: ValidationRule['type'], sanitize: boolean | undefined): unknown {
    if (!sanitize) {
      return value;
    }
    ...
    switch (type) {
      ...
      case 'number':
        return InputSanitizer.sanitizeNumber(value as number | string);
      default:
        return value;
    }
  }
   ...
   */
  static validateRules(value: unknown, rules: ValidationRule, field: string): string[] {
    const errors: string[] = [];
...
export function useInputValidation(schema: ValidationSchema) {
  ...
  const validate = React.useCallback((data: Record<string, unknown>): ValidationResult => {
  ...
  const sanitize = React.useCallback((data: Record<string, unknown>): Record<string, string> => {

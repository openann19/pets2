...
function detectThermalState(): 'normal' | 'warning' | 'critical' {
  try {
    ...
    logger.debug('Thermal monitoring not available:', { error });
  }
...
function detectLowPowerMode(): boolean {
  try {
    ...
    logger.debug('Low power mode detection not available:', { error });
  }
...
function detectSkiaCapability(): boolean {
  try {
    ...
    logger.debug('react-native-skia not available:', { error });
  }

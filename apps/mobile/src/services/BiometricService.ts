...
class BiometricService {
  ...
  async checkBiometricSupport(): Promise<BiometricCapabilities> {
    try {
      ...
      // Get security level if available
      ...
      try {
        ...
        // Security level not available on all platforms/versions
  ...
  async authenticate(reason?: string): Promise<BiometricAuthResult> {
    try {
      ...
      const result = await LocalAuthentication.authenticateAsync({
        promptMessage:
          reason !== undefined && reason !== '' ? reason : 'Authenticate to access PawfectMatch',

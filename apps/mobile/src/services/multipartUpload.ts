...
export async function multipartUpload({
  ...
  const { key, uploadId } = await request<{ key: string; uploadId: string }>(
  ...
  const data = await request<{
    url: string;
    key: string;
    thumbnails: { jpg: string; webp: string };
  }>('/upload/multipart/complete', { method: 'POST', body: { key, uploadId, parts } });
  return data;
}

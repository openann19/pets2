...
export const AdvancedCard: React.FC<AdvancedCardProps> = ({
  ...
  // Haptic Feedback
  const triggerHaptic = useCallback(async (type: 'light' | 'medium' | 'heavy' = haptic) => {
    if (disabled) return;
  ...
  // Hover Animation
  const animateHover = useCallback((hovered: boolean) => {
    if (disabled || loading || isLoading) return;
  ...
  // Handle Action Press
  const handleActionPress = useCallback(async (action: CardAction) => {
  ...
  const renderContent = () => {
    return (
      <View style={[styles.content, { padding: getPaddingValue(padding) }, contentStyle]}>
        {/* Badge */}
        {badge && (
          <View style={[styles.badge, { backgroundColor: badge.backgroundColor || '#ef4444' }]}>
        ...
        {/* Status */}
        {status && (
          <View style={[styles.status, { backgroundColor: status.backgroundColor || '#10b981' }]}>
            <Text style={[styles.statusText, { color: status.color || '#fff' }]}>
              {status.text}
            </Text>
          </View>
        )}

...
export const AdvancedHeader: React.FC<AdvancedHeaderProps> = ({
  ...
  // Handle Button Press
  const handleButtonPress = useCallback(async (button: HeaderButton) => {
    ...
    try {
      ...
      if (button.haptic) {
        await Haptics.impactAsync(
          button.haptic === 'light' ? Haptics.ImpactFeedbackStyle.Light :
          button.haptic === 'medium' ? Haptics.ImpactFeedbackStyle.Medium :
  ...
  const getButtonIcon = (type: HeaderButtonType, customIcon?: string): string => {
    ...
    switch (type) {
      ...
      case 'filter': return 'options-outline';
      ...
      case 'more': return 'ellipsis-vertical';
      default: return 'ellipsis-vertical';
    }
  };

...
function AdvancedButtonComponent({
  ...
  // Haptic Feedback
  const triggerHaptic = useCallback(async (type: keyof typeof HAPTIC_PATTERNS = haptic) => {
  ...
  const renderContent = () => {
    ...
    return (
      <View style={styles.contentContainer}>
        {icon && (
          <Text style={[styles.icon, textStyle, { fontSize: getSizeStyles().minHeight * 0.4 }]}>
            {icon}
          </Text>
        )}
        {title && (
          <Text style={[styles.title, textStyle, { fontSize: getSizeStyles().minHeight * 0.35 }]}>
            {title}
          </Text>
        )}
      </View>

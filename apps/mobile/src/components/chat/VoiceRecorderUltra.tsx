...
interface Props {
  ...
  voiceProcessingService?: {
    ...
    trimAndNormalize: (uri: string, opts: {
      trimThresholdDb: number; trimPaddingMs: number; targetLufs: number;
...
export function VoiceRecorderUltra({
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (backend === "web" && canProcessOnWeb() && (opts.autoTrimSilence || Number.isFinite(opts.normalizeToLufs))) {
        try {
      ...
      if (backend === "native" && voiceProcessingService && (opts.autoTrimSilence || Number.isFinite(opts.normalizeToLufs))) {
        try {
          setIsProcessingAudio(true);
  ...
  }, [backend, chatService, durationMs, isSending, matchId, minDurationSec, onVoiceNoteSent, previewUri, processedBlob, processedUri, transcript]);
...
const styles = StyleSheet.create({
  ...
  recordWrapCancel: {
    ...
    borderWidth: 1, borderColor: Theme.colors.status.error,
  },
  recordButton: {

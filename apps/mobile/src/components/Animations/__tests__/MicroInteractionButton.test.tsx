...
jest.mock('react-native-reanimated', () => {
  ...
  const AnimatedText = React.forwardRef((props: unknown, ref: unknown) => {
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const { children, ...restProps } = props as any;
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
  ...
  const AnimatedScrollView = React.forwardRef((props: unknown, ref: unknown) => {
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const { children, ...restProps } = props as any;
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    return React.createElement(RNScrollView, { ...restProps, ref }, children);
  });
  ...
  const AnimatedModule = {
    ...
    interpolate: jest.fn((value: number, ...args: unknown[]) => {
...
describe('MicroInteractionButton', () => {
  beforeEach(() => {
    jest.clearAllMocks();
    (useReduceMotion as jest.Mock<boolean>).mockReturnValue(false);
    (useSoundEffect as jest.Mock<{
      play: typeof mockPlaySound;
      enabled: boolean;
    ...
    (useHapticFeedback as jest.Mock<{
      triggerHaptic: jest.Mock;
      triggerSuccess: jest.Mock;
      triggerWarning: jest.Mock;
      triggerError: jest.Mock;
    }>).mockReturnValue({
  ...
  describe('Accessibility', () => {
    ...
    it('should have disabled state when disabled', () => {
      ...
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      expect((button.props as any).accessibilityState.disabled).toBe(true);
    });
    ...
    it('should have busy state when loading', () => {
      ...
      const button = getByRole('button');
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      expect((button.props as any).accessibilityState.busy).toBe(true);
    });
    ...
    it('should respect reduced motion', () => {
      (useReduceMotion as jest.Mock<boolean>).mockReturnValue(true);

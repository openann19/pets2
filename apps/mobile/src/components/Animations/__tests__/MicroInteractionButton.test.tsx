...
jest.mock('react-native-reanimated', () => {
  ...
  const typedModule = AnimatedModule as typeof AnimatedModule & { default: typeof AnimatedModule };
  typedModule.default = AnimatedModule;
  
  return {
    default: typedModule.default,
    ...typedModule,
    useSharedValue: jest.fn((value: any) => ({ value })),
    useAnimatedStyle: jest.fn(<T extends () => Record<string, unknown>>(callback: T): ReturnType<T> => {
      try {
    ...
    withSpring: jest.fn(<T>(value: T): T => value),
    withTiming: jest.fn(<T>(value: T): T => value),
    interpolate: jest.fn((value: number, inputRange: number[], outputRange: number[]): number => {
      if (inputRange.length === 0 || outputRange.length === 0) return value;
      const index = inputRange.findIndex((range, i) => i < inputRange.length - 1 && value >= range && value <= inputRange[i + 1]);
      if (index === -1) {
        return value < inputRange[0] ? outputRange[0] : outputRange[outputRange.length - 1];
      }
      const ratio = (value - inputRange[index]) / (inputRange[index + 1] - inputRange[index]);
      return outputRange[index] + (outputRange[index + 1] - outputRange[index]) * ratio;
    }),
    Extrapolate: {
      IDENTITY: 'identity',
      CLAMP: 'clamp',
    ...
    runOnJS: jest.fn(<T extends (...args: unknown[]) => unknown>(fn: T): T => fn),
  };
});
jest.mock('@/theme', () => ({
  useTheme: () => ({
...
describe('MicroInteractionButton', () => {
  beforeEach(() => {
    jest.clearAllMocks();
    (useReduceMotion as unknown as jest.Mock).mockReturnValue(false);
    (useSoundEffect as unknown as jest.Mock).mockReturnValue({
      play: mockPlaySound,
      enabled: true,
      setEnabled: jest.fn(),
    });
    (useHapticFeedback as unknown as jest.Mock).mockReturnValue({
  ...
  describe('Accessibility', () => {
    ...
    it('should respect reduced motion', () => {
      (useReduceMotion as unknown as jest.Mock).mockReturnValue(true);
  ...
  describe('Edge Cases', () => {
    it('should handle empty label', () => {
      ...
      const result = render(<MicroInteractionButton label="" />);
      expect(result).toBeTruthy();
    });

    it('should handle undefined label', () => {
      // Component should render even without label
      const result = render(<MicroInteractionButton />);
      expect(result).toBeTruthy();
    });

...
export const WithMagneticFX = forwardRef<View, WithMagneticFXProps>(({
  ...
  const { handleTouchStart, handleTouchEnd, animatedStyle: magneticStyle } = useMagneticEffect(
...
export const WithShimmerFX = forwardRef<View, WithShimmerFXProps>(({
  ...
  const { animatedStyle: shimmerStyle } = useShimmerEffect(disabled ? 0 : duration);
...
export const WithGradientFX = forwardRef<View, WithGradientFXProps>(({
  ...
  const gradientColors = colors || gradientConfig?.colors || [Theme.colors.primary[500], Theme.colors.primary[400]];

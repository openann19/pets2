...
export const Cropper = forwardRef<CropperHandle, Props>(
  (
    { uri, containerW, containerH, defaultRatio = '4:5', onCropped, showStoryGuides = false },
    ref,
  ) => {
    ...
    const baseScale = useMemo(
      () => (!imgW || !imgH ? 1 : Math.min(containerW / imgW, containerH / imgH)),
    ...
    const focusTo = (rect: Rect) => {
      'worklet';
      const pad = 1.1; // 10% padding around subject

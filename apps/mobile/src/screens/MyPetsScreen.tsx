...
export default function MyPetsScreen({ navigation }: MyPetsScreenProps) {
  const listRef = useRef<FlashList<Pet>>(null);
  ...
  // Pet-first quick actions
  const handleFindPlaymates = useCallback(() => {
    if (allPets.length === 0) {
      haptic.error();
      return;
    }

    // Navigate to playdate discovery for the first pet (or let user choose)
    const primaryPet = allPets.find(p => p.owner === 'currentUser') || allPets[0];
    if (primaryPet) {
      navigation.navigate('PlaydateDiscovery', { petId: primaryPet._id });
    }
  }, [allPets, navigation]);

  const handleViewMap = useCallback(() => {
    navigation.navigate('PetFriendlyMap', { filter: 'all' });
  }, [navigation]);

  const handleHealthCheck = useCallback(() => {
    // Navigate to health overview or first pet's health passport
    const primaryPet = allPets.find(p => p.owner === 'currentUser') || allPets[0];
    if (primaryPet) {
      navigation.navigate('HealthPassport', { petId: primaryPet._id });
    }
  }, [allPets, navigation]);

  const handleLostPetAlert = useCallback(() => {
    navigation.navigate('LostPetAlert');
  }, [navigation]);
  ...
  const renderEmptyState = () => (
    <EmptyStates.NoPets
      ...
      message="Create your first pawfile to find playmates, explore pet-friendly places, and track your pet's health journey!"
      actionLabel="Create Your First Pawfile"
      onAction={() => {
        haptic.confirm();
        navigation.navigate('CreatePet');
      }}
  ...
  return (
    <ScreenShell
      header={
        <AdvancedHeader
          {...HeaderConfigs.glass({
            title: 'My Pawfiles',
            subtitle: allPets.length > 0 ? `${allPets.length} pet${allPets.length !== 1 ? 's' : ''}` : undefined,
            rightButtons: [
              {
                type: 'paw',
                iconName: 'paw-outline',
                onPress: handleFindPlaymates,
                variant: 'secondary',
                haptic: 'light',
    ...
    >
      <FlashList
        ...
        estimatedItemSize={200} // Estimated pet card height
            ...
            <Animated.View entering={FadeInDown.duration(200)}>
              <View style={styles.listHeader}>
                <Text style={styles.listHeaderText}>
                  Your Pawfiles - Ready for adventures! 🐾
                </Text>
              </View>
            </Animated.View>
          ) : null

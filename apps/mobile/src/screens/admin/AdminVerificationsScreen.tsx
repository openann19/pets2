...
function AdminVerificationsScreen({ navigation }: AdminVerificationsScreenProps): JSX.Element {
  ...
  const handleVerificationAction = useCallback(async (
          ...
          prev.map(verification => {
            ...
            const updated: Verification = {
              ...
              status: action === 'approve' ? 'approved' :
                action === 'reject' ? 'rejected' : 'requires_info',
  ...
  const handleRejectWithReason = useCallback((verification: Verification) => {
    Alert.prompt(
      'Reject Verification',
  ...
  const handleRequestInfo = useCallback((verification: Verification) => {
    Alert.prompt(
  ...
  const getStatusColor = (status: Verification['status']) => {
    switch (status) {
      ...
      case 'requires_info': return '#8B5CF6';
  ...
  const getVerificationTypeIcon = (type: Verification['type']) => {
    switch (type) {
      ...
      case 'pet_ownership': return 'paw';
      case 'veterinary': return 'medical';
      case 'breeder': return 'ribbon';
  ...
  const renderVerification = useCallback(({ item }: { item: Verification }) => (
    ...
    >
      ...
      <View style={styles.cardContent}>
        ...
        {item.expiresAt ? <Text style={[styles.expiresAt, { color: colors.error }]}>
  ...
  ), [colors, handleVerificationAction, handleRejectWithReason, handleRequestInfo]);
        ...
        <View style={[styles.modalContent, { backgroundColor: colors.card }]}>
          ...
          <View style={styles.modalBody}>
            <View style={styles.verificationInfo}>
              <Text style={[styles.infoLabel, { color: colors.textSecondary }]}>User:</Text>
            ...
            <Text style={[styles.documentsHeader, { color: colors.text }]}>Documents:</Text>
            <FlatList
              data={selectedVerification.documents}
              keyExtractor={(item) => item.id}
              renderItem={({ item }) => (
                <View style={[styles.documentItem, { backgroundColor: colors.background }]}>

...
function AdminVerificationsScreen({
    ...
    async (refresh = false) => {
      try {
        ...
        if (response?.success && response.data) {
          ...
          setVerifications(
            Array.isArray(responseData.verifications) ? responseData.verifications : [],
          );
        }
      } catch (error) {
    ...
    async (
      ...
      try {
        ...
        if (action === 'approve') {
          ...
          response = await _adminAPI.requestVerificationInfo(
            verificationId,
        ...
        if (response?.success) {
          setVerifications((prev) =>
            prev.map((verification) => {
              ...
              const statusMap: Record<
                ...
                'approved' | 'rejected' | 'requires_info'
              > = {
                approve: 'approved',
                reject: 'rejected',
          ...
          Alert.alert('Success', `Verification ${action.replace('_', ' ')}d successfully`);
        ...
        errorHandler.handleError(
          error instanceof Error ? error : new Error(`Failed to ${action} verification`),
          {
            component: 'AdminVerificationsScreen',
      ...
      Alert.prompt(
        ...
        [
          { text: 'Cancel', style: 'cancel' },
          {
            ...
            onPress: (reason) => {
              if (reason?.trim()) {
                void handleVerificationAction(verification.id, 'reject', reason.trim());
              }
            },
          },
        ],
      ...
      Alert.prompt(
        ...
        [
          { text: 'Cancel', style: 'cancel' },
          {
            ...
            onPress: (info) => {
              if (info?.trim()) {
                void handleVerificationAction(verification.id, 'request_info', info.trim());
              }
            },
          },
        ],
  ...
  const renderVerification = ({ item }: { item: Verification }) => (
    <TouchableOpacity
      style={StyleSheet.flatten([styles.verificationCard, { backgroundColor: colors.surface }])}
      testID="AdminVerificationsScreen-button-2"
    ...
    >
      <View style={styles.cardHeader}>
        <View style={styles.userInfo}>
          <View style={styles.typeContainer}>
            ...
            />
            <Text
              style={StyleSheet.flatten([styles.verificationType, { color: colors.onSurface }])}
            >
              {item.type.replace('_', ' ').toUpperCase()}
            </Text>
          </View>
          <Text style={StyleSheet.flatten([styles.userName, { color: colors.onSurface }])}>
            {item.userName}
          </Text>
          <Text style={StyleSheet.flatten([styles.userEmail, { color: colors.onMuted }])}>
            {item.userEmail}
          </Text>
        </View>
      ...
      <View style={styles.cardContent}>
        <Text style={StyleSheet.flatten([styles.submittedAt, { color: colors.onMuted }])}>
        ...
        <View style={styles.documentsInfo}>
          ...
          />
          <Text style={StyleSheet.flatten([styles.documentsCount, { color: colors.onMuted }])}>
        ...
        {item.expiresAt ? (
          <Text style={StyleSheet.flatten([styles.expiresAt, { color: theme.colors.danger }])}>
      ...
      {item.status === 'pending' && (
        <View style={styles.quickActions}>
          <TouchableOpacity
            style={StyleSheet.flatten([styles.quickActionButton, styles.approveButton])}
            testID="AdminVerificationsScreen-button-2"
            accessibilityLabel="Interactive element"
            accessibilityRole="button"
          ...
          <TouchableOpacity
            style={StyleSheet.flatten([styles.quickActionButton, styles.infoButton])}
            testID="AdminVerificationsScreen-button-2"
            accessibilityLabel="Interactive element"
            accessibilityRole="button"
            onPress={() => {
          ...
          <TouchableOpacity
            style={StyleSheet.flatten([styles.quickActionButton, styles.rejectButton])}
            testID="AdminVerificationsScreen-button-2"
            accessibilityLabel="Interactive element"
            accessibilityRole="button"
            onPress={() => {
  ...
  const renderVerificationModal = () => {
    ...
    return (
      <View style={styles.modalOverlay}>
        <View style={StyleSheet.flatten([styles.modalContent, { backgroundColor: colors.card }])}>
          <View style={styles.modalHeader}>
            <Text style={StyleSheet.flatten([styles.modalTitle, { color: colors.onSurface }])}>
          ...
          <View style={styles.modalBody}>
            <View style={styles.verificationInfo}>
              <Text style={StyleSheet.flatten([styles.infoLabel, { color: colors.onMuted }])}>
                User:
              </Text>
              <Text style={StyleSheet.flatten([styles.infoValue, { color: colors.onSurface }])}>
            ...
            <Text style={StyleSheet.flatten([styles.documentsHeader, { color: colors.onSurface }])}>
              Documents:
            </Text>
            <FlatList
              ...
              renderItem={({ item }) => (
                ...
                >
                  ...
                  <View style={styles.documentInfo}>
                    <Text
                      style={StyleSheet.flatten([styles.documentName, { color: colors.onSurface }])}
                    >
                      {item.name}
                    </Text>
                    <Text
                      style={StyleSheet.flatten([styles.documentType, { color: colors.onMuted }])}
                    >
                      {item.type.replace('_', ' ')}
                    </Text>
                  </View>
          ...
          {selectedVerification.status === 'pending' && (
            <View style={styles.modalActions}>
              <TouchableOpacity
                ...
                accessibilityLabel="Interactive element"
                accessibilityRole="button"
                onPress={() => handleVerificationAction(selectedVerification.id, 'approve')}
              >
                <Ionicons
                  name="checkmark"
                  size={20}
              ...
              <TouchableOpacity
                style={StyleSheet.flatten([styles.actionButton, styles.infoButton])}
                testID="AdminVerificationsScreen-button-2"
                accessibilityLabel="Interactive element"
                accessibilityRole="button"
                onPress={() => {

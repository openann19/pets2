  ...
  const loadChats = useCallback(
    async (options?: { force?: boolean }) => {
      try {
        ...
        const err =
          error instanceof Error ? error : new Error("Failed to load chats");
    ...
    return chats.filter((chat) => {
      ...
      if (query.length > 0) {
        const participantNames = [
          chat.participants.user1.name,
          chat.participants.user2.name,
        ]
          .join(" ")
          .toLowerCase();
  ...
  const onChatBlock = useCallback(
    async (chatId: string) => {
      try {
        ...
        const err =
          error instanceof Error ? error : new Error("Failed to block chat");
  ...
  const onChatWarn = useCallback(
    async (chatId: string) => {
      try {
        ...
        Alert.alert(
          "Warning Sent",
          "Both users have been warned about community guidelines.",
        );
        ...
        const err =
          error instanceof Error ? error : new Error("Failed to send warning");

...
export function useAdminAnalyticsScreen({
  ...
  const generateMockAnalytics = useCallback((range: typeof timeRange): AnalyticsData => {
    const baseMultiplier = range === '7d' ? 1 : range === '30d' ? 4 : range === '90d' ? 12 : 52;
  ...
  const loadAnalytics = useCallback(
    async (options?: { force?: boolean }) => {
      try {
        ...
        const err = error instanceof Error ? error : new Error('Failed to load analytics');
  ...
  const onExportData = useCallback(
    async (format: 'csv' | 'pdf') => {
      try {
        ...
        const err = error instanceof Error ? error : new Error('Failed to export data');
  ...
  const chartData = useMemo(() => {
    ...
    return {
      userGrowth: {
        ...
        datasets: [
          {
            data: [12000, 13500, 14200, 14800, 15200, analytics.users.totalUsers],
            color: () => '#3B82F6',
          },
        ],
      },
      revenueTrend: {
        ...
        datasets: [
          {
            data: [8500, 9200, 10100, 11200, 11800, analytics.revenue.revenueThisMonth],
            color: () => '#10B981',
          },
        ],
      },
      matchSuccess: {
        ...
        datasets: [
          {
            data: [
              analytics.matches.totalMatches * (analytics.matches.matchSuccessRate / 100),
              analytics.matches.totalMatches * (1 - analytics.matches.matchSuccessRate / 100),
            ],

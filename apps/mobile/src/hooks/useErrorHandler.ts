...
export const useErrorHandler = () => {
  ...
  const handleNetworkError = useCallback((error: NetworkError | Error | string, context?: string, onRetry?: () => void) => handleError(error, context, {
    showAlert: true,
    logError: true,
  ...
  const handleAuthError = useCallback((context?: string) => handleError('Authentication failed. Please log in again.', context, {
    showAlert: true,
    logError: true
  }), [handleError]);

  const handleValidationError = useCallback((errors: ValidationError[], context?: string) => {
  ...
  const handleOfflineError = useCallback((context?: string, onRetry?: () => void) => handleError('You appear to be offline. Please check your connection.', context, {
    showAlert: true,

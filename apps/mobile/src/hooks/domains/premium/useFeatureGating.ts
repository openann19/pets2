...
export const useFeatureGating = (): UseFeatureGatingReturn => {
      ...
      try {
        ...
        if (
          ...
          return {
            ...
            reason: limit === 0 ? `No ${FEATURE_DISPLAY_NAMES[feature]} remaining` : undefined,
          };
        } else {
          ...
          return {
            ...
            reason: canUse ? undefined : `Premium required for ${FEATURE_DISPLAY_NAMES[feature]}`,
          };
        }
      } catch (error) {
  ...
  const getUpgradeMessage = useCallback((feature: keyof PremiumLimits): string => {

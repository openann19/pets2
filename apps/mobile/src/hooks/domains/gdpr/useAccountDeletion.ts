...
export function useAccountDeletion(): UseAccountDeletionReturn {
  ...
  const requestDeletion = useCallback(
    async (password: string, reason?: string, feedback?: string): Promise<boolean> => {
      ...
      try {
        const result = await gdprService.requestAccountDeletion(password, reason, feedback);
        ...
        if (result.success) {
          ...
          logger.info("Account deletion requested", { deletionId: result.deletionId });
          return true;
        }
        ...
        logger.error("Account deletion request failed:", { error: errorMessage });
  ...
  const cancelDeletion = useCallback(async (): Promise<boolean> => {
    ...
    try {
      ...
      setError(errorMessage);
      logger.error("Failed to cancel account deletion:", { error: errorMessage });
      return false;
    } finally {
      setIsDeleting(false);
    }

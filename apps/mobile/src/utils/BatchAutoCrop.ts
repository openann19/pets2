...
export async function batchAutoCrop(
  items: BatchItem[],
  ratio: string = "4:5",
  options: { eyeWeight?: number; padPct?: number; concurrency?: number; onProgress?: (done: number, total: number, last?: BatchResult) => void } = {}
): Promise<BatchResult[]> {
  ...
  async function worker() {
    while (queue.length) {
      ...
      try {
        const [suggestion] = await AutoCropEngine.suggestCrops(item.uri, [ratio], { eyeWeight, padPct });
  ...
  return results.sort((a, b) => (String(a.input.id ?? a.input.uri)).localeCompare(String(b.input.id ?? b.input.uri)));
}

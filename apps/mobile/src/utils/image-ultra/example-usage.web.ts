...
export async function fullPipelineExample(fileBlob: Blob) {
  ...
  thumbCanvas
    .getContext('2d')!
    .drawImage(result.canvas, 0, 0, thumbCanvas.width, thumbCanvas.height);
  const webpThumb = await new Promise<Blob>((resolve) => {
    thumbCanvas.toBlob(
      (blob) => {
        resolve(blob!);
...
export async function individualProFeaturesExample(fileBlob: Blob) {
  const { toneMapHighlights, clarityLocalContrast, vignetteCorrect, applyNoisePreset } =
    await import('./filters_extras');

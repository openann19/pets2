...
export function DoubleTapLikePlus({
  ...
  const pScale = items.map(() => useSharedValue(0));
  const pX = items.map(() => useSharedValue(0));
  const pY = items.map(() => useSharedValue(0));
  const pOpacity = items.map(() => useSharedValue(0));
  const pRotate = items.map(() => useSharedValue(0));
      ...
      <Animated.View style={[styles.wrap, style, containerStyle]}>
        ...
        {items.map((i) => {
          const s = useAnimatedStyle(() => ({
            ...
            top: "50%",
            transform: [
              { translateX: -heartSize / 2 + pX[i].value },
              { translateY: -heartSize / 2 + pY[i].value },
              { rotate: `${pRotate[i].value}deg` },
              { scale: pScale[i].value },
            ],
            opacity: pOpacity[i].value,
          }));
          return (

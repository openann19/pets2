...
export default class MapSocketServer {
  private io: any;
  private activePins: Map<string, any>;
  ...
  setupSocketHandlers() {
    this.io.on('connection', (socket: any) => {
      ...
      socket.on('activity:end', (data: any) => {
        const { pinId } = data;
      ...
      socket.on('nearby:request', (data: any) => {
      ...
      socket.on('match:notify', (data: any) => {
        ...
        if (socket.userId) {
          ...
          setTimeout(() => {
            const userPins = Array.from(this.activePins.entries())
              .filter(([, pin]: [string, any]) => pin.ownerId === socket.userId);

            userPins.forEach(([pinId]: [string, any]) => {
              this.activePins.delete(pinId);
              this.io.emit('pin:remove', pinId);
            });
          }, 5 * 60 * 1000);
  ...
  startLocationSimulation() {
    ...
    setInterval(() => {
      ...
      const expiredPins = Array.from(this.activePins.entries())
        .filter(([, pin]: [string, any]) => new Date(pin.expiresAt) <= now);

      expiredPins.forEach(([pinId]: [string, any]) => {
        this.activePins.delete(pinId);
        this.io.emit('pin:remove', pinId);
      });
  ...
  generateHeatmapData() {
    ...
    const pins = Array.from(this.activePins.values())
      .filter((pin: any) => new Date(pin.expiresAt) > new Date());
  ...
  getStats() {
    ...
    const activePins = Array.from(this.activePins.values())
      .filter((pin: any) => new Date(pin.expiresAt) > now);

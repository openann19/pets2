 ...
 * Pet Schema
 */
const petSchema = new Schema<IPet, IPetModel, IPetMethods>({
  // Owner
  owner: {
    type: String,
    ref: 'User',
...
petSchema.virtual('ageInMonths').get(function(this: IPetDocument): number {
  return this.age * 12;
});

// Virtual for primary photo
petSchema.virtual('primaryPhoto').get(function(this: IPetDocument): IPetPhoto | null {
  const primary = this.photos.find((photo: IPetPhoto) => photo.isPrimary);
...
petSchema.pre('save', function(this: IPetDocument, next) {
  ...
  if (this.photos && this.photos.length > 0) {
    const primaryPhotos = this.photos.filter((photo: IPetPhoto) => photo.isPrimary);
    if (primaryPhotos.length === 0) {
      this.photos[0]!.isPrimary = true;
    } else if (primaryPhotos.length > 1) {
      this.photos.forEach((photo: IPetPhoto, index: number) => {
...
// Static methods
petSchema.statics.findBySpeciesAndIntent = function(species?: string, intent?: string) {

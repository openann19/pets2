...
router.post('/:id/approve', async (req, res) => {
  try {
    ...
    // Notify user
    await notifyUserPhotoApproved(moderation.userId, moderation);
...
router.post('/:id/reject', async (req, res) => {
  try {
    ...
    // Notify user
    await notifyUserPhotoRejected(moderation.userId, moderation, reason, category);
  ...
  try {
    ...
    for (const id of ids) {
      try {
        ...
        if (moderation) {
          await moderation.approve(req.user._id, notes);
...
async function notifyUserPhotoApproved(userId, moderation) {
  // TODO: Implement notification service
  logger.info('User notification: photo approved', { userId, moderationId: moderation._id });
}

async function notifyUserPhotoRejected(userId, moderation, reason, category) {
  // TODO: Implement notification service
  logger.info('User notification: photo rejected', { userId, moderationId: moderation._id, category });
}

// Export setSocketIO for server.js to inject Socket.io instance

...
import { zodValidate } from '../src/middleware/zodValidator';
import { authenticateToken } from '../src/middleware/auth';
import { uploadRateLimiter, strictRateLimiter } from '../src/middleware/globalRateLimit';
import { storyDailyLimiter } from '../src/middleware/storyDailyLimiter';
import * as storiesController from '../src/controllers/storiesController';

// Prefer shared core schemas; fall back to local server schemas if interop fails
let createStorySchema: any, replySchema: any;
try {
  const schemas = require('@pawfectmatch/core/schemas');
  createStorySchema = schemas.storyCreateSchema;
  replySchema = schemas.storyReplySchema;
} catch (e) {
  const localSchemas = require('../src/schemas/storySchemas');
  createStorySchema = localSchemas.createStorySchema;
  replySchema = localSchemas.replySchema;
}

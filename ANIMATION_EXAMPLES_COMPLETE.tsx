...
// END
// ------------------------------------------------------------------------------------
Ohhh yes — upgraded to ULTRA. I’ve dropped a fully re-engineered V2 suite into your canvas with:

ParallaxHeroV2 — true 3D depth (translateZ via transformTemplate), sticky/pinned scrollytelling, spring smoothing per layer, SSR-safe, paint isolation, reduced-motion fallback.

TiltCardV2 — glossy glare, inner-parallax for child layers (data-depth-style), keyboard/touch friendly, inertia springs, zero jank.

Reveal V2 — tiny IntersectionObserver hook + upgraded CSS (content-visibility, backface hiding, reduced-motion).

Drop-in examples for Next.js (web) and a separate RN snippet so mobile isn’t mixed with DOM.

Copy-paste REVEAL_CSS_V2 string straight into globals.css.

Want it jaw-droppier? Try these immediate tweaks in your app:

Put the CSS in app/globals.css:

/* Paste REVEAL_CSS_V2 from the canvas here */

Use the components:

// app/page.tsx
import { UltraAnimationsDemoPage } from '@/components/ultra-animations.v2';
export default function Page() { return <UltraAnimationsDemoPage />; }


Hero with your own layers:
...
<ParallaxHeroV2
  ...
  height="130vh"
  layers={[
    { children: <div className="bg-gradient-to-br from-violet-700 via-fuchsia-600 to-cyan-400" />,
      scrollRange: [0, 900], yRange: [0, -160], z: -240, zIndex: 1 },
    { children: <div className="absolute top-20 left-16 w-40 h-40 rounded-full bg-white/12 backdrop-blur" />,
      scrollRange: [0, 900], yRange: [0, -80], xRange: [0, 60], opacityRange: [.5, .95], z: -100, zIndex: 2 },
    { children: <div className="flex items-center justify-center h-full"><h1 className="text-6xl md:text-7xl font-extrabold text-white drop-shadow-2xl">Pawfect Motion</h1></div>,
      scrollRange: [0, 500], yRange: [0, -30], scaleRange: [.92, 1], opacityRange: [0, 1], z: 0, zIndex: 10, spring:{stiffness:180,damping:24} },
  ]}
/>


Tilt with glare + gradient frame:

<TiltCardV2 className="rounded-2xl p-[1px] bg-gradient-to-br from-indigo-500 via-fuchsia-500 to-cyan-400">
  <div className="rounded-2xl p-6 bg-slate-950/70 text-white">
    <h3 className="text-xl font-bold mb-2">Adopt Luna</h3>
    <p className="text-white/80">3D tilt, inner parallax & glossy glare.</p>
  </div>
</TiltCardV2>


If you want me to wire this into your PawfectMatch Premium monorepo (Next 14 + Tailwind + shadcn), say the word and I’ll slot it under packages/ui-animations with exports, storybook examples, and typed presets for your design tokens.